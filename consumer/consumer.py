...
def callback(ch, method, properties, body):
            ...
            t = Transaction(time=int(single_row['Time']), amount=float(single_row['Amount']))   # type: ignore # type: ignore
            try:
                single_row.drop(['Amount','Class'],axis=1,inplace=True)
            except Exception as e:
                print(f"Error {e} while dropping columns")
            answer=model.predict(single_row)[0]  # type: ignore
            t.Class=int(answer)  # type: ignore
            s.add(t)  # type: ignore
...
def main():
        ...
        channel.exchange_declare(exchange='fraudDetection',exchange_type='direct')  # type: ignore
